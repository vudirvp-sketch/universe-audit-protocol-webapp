...
2. Если >50% пунктов имеют INSUFFICIENT_DATA — балл = 0%, гейт НЕ ПРОЙДЕН. Это означает, что для надёжной оценки недостаточно данных. Не завышай балл при обилии INSUFFICIENT_DATA.
3. Порог гейта зависит от режима аудита (60% конфликт, 50% кирё, 55% гибрид)
4. Балл = (пройдено / (пройдено + провалено)) × 100%
5. Используй INSUFFICIENT_DATA только когда в тексте ДЕЙСТВИТЕЛЬНО нет информации для оценки. Не используй его как «мягкий провал».
